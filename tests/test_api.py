...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    ...
    not_exposed = [
        ...
        if obj.name not in griffe_pydantic.__all__ or not hasattr(griffe_pydantic, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in griffe_pydantic.__all__:
...
def test_inventory_matches_api(
    ...
    ignore = ("model_ext", "model_noext")
    ignore_paths = {
        "griffe_pydantic.common",
        "griffe_pydantic.extension",
        "griffe_pydantic.dynamic",
        "griffe_pydantic.static",
    }

    not_in_api = []
    public_api_paths = {obj.path for obj in public_objects}
    public_api_paths.add("griffe_pydantic")
    for item in inventory.values():
        ...
        if (
            ...
            and (item.name == "griffe_pydantic" or item.name.startswith("griffe_pydantic."))
        ):
